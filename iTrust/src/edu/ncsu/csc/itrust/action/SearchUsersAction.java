...
public class SearchUsersAction {
	...
	/**
	 * @author Jay Patel - added fuzzy search based on MID as well
	 * Search for all patients with first name and last name given in parameters.
	 * @param query query
	...
	public List<PatientBean> fuzzySearchForPatients(String query, boolean allowDeactivated) {
		...
		if(query!=null && query.length()>0 && !query.startsWith("_")){
			...
			for(String q : subqueries){
				try {
					...
					try{
						long mid = Long.valueOf(q); //Takes query puts it into long format
						List<PatientBean> lmid = patientDAO.fuzzySearchForPatientsWithMID(mid); //fuzzy searches for patients based on MID
						patients[i].addAll(lmid); //adds all patients containing that number
		...
		Collections.reverse(results); //Reverses the order of the list for clients specifications
